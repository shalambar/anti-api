...
import { isLoopbackRequest, isRemoteControlPlaneAllowed } from "./lib/local-request"
...
server.use(cors({
    ...
    // ANTI_API_ALLOW_REMOTE_CONTROL_PLANE=1 时允许远程面板所在来源（如 Railway 域名）
    origin: (origin) => {
        if (!origin) return origin
        try {
            const host = new URL(origin).hostname
            if (host === "localhost" || host === "127.0.0.1" || host === "::1") return origin
            if (isRemoteControlPlaneAllowed()) return origin
        } catch { }
        return null
    },
}))
server.use(async (c, next) => {
    if (!isRemoteControlPlaneAllowed() && !isLoopbackRequest(c.req.raw)) {
